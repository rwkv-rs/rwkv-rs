...
use crate::{
    access::http_api::{CompletionRequest, HttpApiService, HttpApiState},
    auth::check_api_key,
};
...
pub async fn completions(
    ...
    if run.stream_requested {
        ...
        tokio::spawn(async move {
            ...
            #[cfg(feature = "trace")]
            tracing::info!(completion_id = %run.id, model = %run.model, "completion stream opened");
            while let Some(ev) = run.rx.recv().await {
                match ev {
                    crate::inference_core::EngineEvent::Output(delta) => {
                        ...
                        {
                            #[cfg(feature = "trace")]
                            tracing::warn!(completion_id = %run.id, "sse receiver dropped");
                            break;
                        }
                    }
                    crate::inference_core::EngineEvent::Done(meta) => {
                        ...
                        {
                            #[cfg(feature = "trace")]
                            tracing::warn!(completion_id = %run.id, "sse receiver dropped before final chunk");
                            break;
                        }
                        let _ = sse_tx
            ...
            #[cfg(feature = "trace")]
            tracing::info!(completion_id = %run.id, emitted_chunks, "completion stream closed");
        });

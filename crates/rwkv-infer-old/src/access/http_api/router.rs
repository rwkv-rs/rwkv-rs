...
pub struct HttpApiState {
    ...
    pub runtime_manager: Arc<LoadedModelRegistry>,
}

pub struct HttpApiRouterBuilder {
    app_state: HttpApiState,
}

impl HttpApiRouterBuilder {
    pub fn new(app_state: HttpApiState) -> Self {
        Self { app_state }
    }
...
async fn models(State(app_state): State<HttpApiState>) -> Json<ModelListResponse> {
    let api = HttpApiService::new(app_state.runtime_manager.clone());
    Json(api.models())
}

pub type AppState = HttpApiState;
pub type RouterBuilder = HttpApiRouterBuilder;

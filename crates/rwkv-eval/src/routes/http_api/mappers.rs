use super::{
    ...
    error::ApiError,
    json::{build_task_progress, parse_sampling_summary, parse_score_summary},
};
use crate::{
    db::{
        BenchmarkRecord,
    ...
    services::admin::health::HealthTargetResult,
};
...
pub(crate) fn to_task_resource(record: &TaskRecord) -> Result<TaskResource, ApiError> {
    let sampling = parse_sampling_summary(&record.sampling_config_json)?;
    ...
    Ok(TaskResource {
        ...
        progress: build_task_progress(record, &sampling),
        sampling,

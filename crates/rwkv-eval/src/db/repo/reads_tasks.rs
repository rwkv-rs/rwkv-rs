    ...
    let mut builder = QueryBuilder::<Postgres>::new(
        ...
        base AS (
            SELECT
                ...
                COALESCE(a.attempts_passed, 0) AS attempts_passed,
                COALESCE(a.attempts_failed, 0) AS attempts_failed,
                COALESCE(a.attempts_with_checker, 0) AS attempts_with_checker,
                COALESCE(a.attempts_missing_checker, 0) AS attempts_missing_checker,
                COALESCE(a.attempts_needing_human_review, 0) AS attempts_needing_human_review
            FROM task t
            ...
            LEFT JOIN attempt_stats a ON a.task_id = t.task_id
            WHERE 1 = 1
        "#,
    );

    if let Some(config_path) = filter.config_path.as_ref() {
        builder
            .push(" AND t.config_path IS NOT DISTINCT FROM ")
            .push_bind(config_path);
    }
...
pub async fn get_task_detail(db: &Db, task_id: i32) -> Result<Option<TaskDetailRecord>, String> {
    ...
    Ok(Some(TaskDetailRecord {
        ...
        attempts_passed: task.attempts_passed,
        attempts_failed: task.attempts_failed,
        attempts_with_checker: task.attempts_with_checker,
        attempts_missing_checker: task.attempts_missing_checker,
        attempts_needing_human_review: task.attempts_needing_human_review,
        task,
    }))
}

...
pub async fn run_checker(
    ...
    retry_checker(checker_model_name, || async {
        run_checker_once(
            checker_client,
            checker_model_name,
            context,
...
async fn run_checker_once(
        ...
        messages: vec![
            CheckerMessage {
                ...
                content: concat!(
                    ...
                    "Keep `reason` concise and no longer than 200 characters.\n",
    ...
    parse_checker_output(content)
}

async fn retry_checker<T, F, Fut>(checker_model_name: &str, mut operation: F) -> Result<T, String>
where
    F: FnMut() -> Fut,
    ...
    for attempt in 1..=3 {
        match operation().await {
            ...
            Err(err) if attempt < 3 => {
                eprintln!(
                    "checker failed on attempt {attempt}/3, retrying: {err}; model={checker_model_name}"
                );
            }
            Err(err) => {
                return Err(format!(
                    "checker failed after 3 attempts: {err}; model={checker_model_name}"
                ));
            }
        }
    }

    panic!("unreachable checker retry loop")
}

fn parse_checker_output(content: &str) -> Result<CheckerOutput, String> {
...
mod tests {
    ...
    #[test]
    fn parses_valid_checker_output() {
        let output = parse_checker_output(
            r#"{
                "answer_correct": false,
                "instruction_following_error": true,
        ...
        assert!(!output.answer_correct);
        assert!(output.instruction_following_error);
        assert!(output.reasoning_logic_error);
        assert_eq!(output.reason, "brief reason");
    }

    #[test]
    fn rejects_invalid_checker_json() {
        let err = parse_checker_output("not valid json").unwrap_err();
        assert!(err.contains("invalid checker json"));
    }

    #[test]
    fn rejects_empty_reason() {
        let err = parse_checker_output(
            r#"{
                "answer_correct": false,
        ...
        assert_eq!(err, "checker returned empty `reason`");
    }

    #[tokio::test]
    async fn retries_until_success() {
        let attempts = Arc::new(AtomicUsize::new(0));
        let result = retry_checker("checker-model", {
    ...
    #[tokio::test]
    async fn returns_final_error_after_three_attempts() {
        let attempts = Arc::new(AtomicUsize::new(0));
        let err = retry_checker("checker-model", {
            let attempts = Arc::clone(&attempts);
            move || {
                let attempts = Arc::clone(&attempts);
                async move {
                    let current = attempts.fetch_add(1, Ordering::SeqCst) + 1;
                    Err::<(), _>(format!("failed attempt {current}"))
                }
            }
        })
        .await
        ...
        assert_eq!(
            err,
            "checker failed after 3 attempts: failed attempt 3; model=checker-model"
        );
    }
}

...
static BRUMO25_INFO: BenchmarkInfo = BenchmarkInfo {
    name: BenchmarkName("brumo25"),
    field: Field::Maths,
    display_name: "Brumo25",
...
mod tests {
    ...
    async fn downloads_and_reads_dataset() {
        crate::datasets::assert_benchmark_download_load_and_read(&BRUMO25_INFO).await;
    }
}
...
#[async_trait]
impl Benchmark for Brumo25 {
    fn load(&mut self) -> bool {
        self.test.clear();

        let parquet_paths = collect_files_with_extension(
            self.dataset_root.join("brumo25/default/train"),
            "parquet",
        );

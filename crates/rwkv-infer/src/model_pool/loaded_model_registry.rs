...
pub struct LoadedModelRegistry {
    ...
    request_router: RwLock<Arc<ModelRequestRouter>>,
...
impl LoadedModelRegistry {
    pub fn bootstrap(
        ...
        let request_router = Arc::new(ModelRequestRouter::new(groups, model_vocab_sizes)?);
        ...
        Ok(Self {
            ...
            request_router: RwLock::new(request_router),
            factory,
    ...
    pub fn current_request_router(&self) -> Arc<ModelRequestRouter> {
        self.request_router
            .read()
            .expect("loaded model registry request_router lock poisoned")
            .clone()
    }

    pub fn model_names(&self) -> Vec<String> {
        self.current_request_router().model_names()
    }
    ...
    pub async fn reload_models(
        ...
        let current_request_router = self.current_request_router();
        let mut merged_groups = current_request_router.clone_model_groups();
        let mut merged_vocab_sizes = current_request_router.clone_model_vocab_sizes();

        for removed in &removed_model_names {
            merged_groups.remove(removed);
            merged_vocab_sizes.remove(removed);
        }
        ...
        if !rebuild_models.is_empty() {
            ...
            for (model_name, group) in rebuilt_groups {
                merged_groups.insert(model_name.clone(), group);
            }
            for (model_name, model_cfg) in model_cfgs {
                merged_vocab_sizes.insert(model_name, model_cfg.vocab_size);
            }
        }
        ...
        let new_request_router =
            Arc::new(ModelRequestRouter::new(merged_groups, merged_vocab_sizes)?);
        ...
        if !patch.dry_run {
            ...
            *self
                ...
                .write()
                .expect("loaded model registry request_router lock poisoned") =
                new_request_router.clone();
        }

        let active_model_names = if patch.dry_run {
            current_request_router.model_names()
        } else {
            new_request_router.model_names()
        };
...
pub type RuntimeManager = LoadedModelRegistry;

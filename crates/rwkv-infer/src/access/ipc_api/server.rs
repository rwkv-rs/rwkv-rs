...
impl IpcServer {
    pub fn from_runtime_manager(
        ...
        Ok(Self {
            config,
            api: HttpApiService::new(runtime_manager),
            auth_cfg,
        ...
        ResponseHeader: Debug + Default + ZeroCopySend,
    {
        let request = match decode_request(active_request.payload()) {
            ...
            Err(e) => {
                let payload = encode_json(
                    &crate::access::http_api::OpenAiErrorResponse::from_infer_error(&e),
                )?;
                self.send_frame(
                    active_request,
                    IpcResponse {
        ...
        if self.config.require_api_key && route_requires_auth(request.route) {
            if let Err(auth_err) = check_api_key_token(request.api_key.as_deref(), &self.auth_cfg) {
                let payload = encode_json(
                    &crate::access::http_api::OpenAiErrorResponse::unauthorized(auth_err.message()),
                )?;
                self.send_frame(
                    active_request,
                    IpcResponse {
    ...
    where
        ...
        ResponseHeader: Debug + Default + ZeroCopySend,
    {
        let mut completion = decode_json::<crate::access::http_api::CompletionRequest>(
            &request.payload,
            "completion request",
        )?;
        ...
        if run.stream_requested {
            ...
            loop {
                let ev = self.runtime_handle.block_on(async { run.rx.recv().await });
                match ev {
                    Some(crate::inference_core::EngineEvent::Output(delta)) => {
                        let chunk = run.chunk(&delta, text_offset, None);
                        text_offset += delta.text.chars().count();
                        let payload = encode_json(&chunk)?;
                        self.send_data(active_request, &request, payload, 200)?;
                    }
                    Some(crate::inference_core::EngineEvent::Done(meta)) => {
                    ...
                    Some(crate::inference_core::EngineEvent::Error(msg)) => {
                        return self.send_infer_error(
                            active_request,
                            &request,
    ...
    where
        ...
        ResponseHeader: Debug + Default + ZeroCopySend,
    {
        let mut chat = decode_json::<crate::access::http_api::ChatCompletionRequest>(
            &request.payload,
        ...
        if run.stream_requested {
            ...
            let role_payload = encode_json(&run.stream_role_chunk())?;
            self.send_data(active_request, &request, role_payload, 200)?;
    ...
    where
        ...
        ResponseHeader: Debug + Default + ZeroCopySend,
    {
        let req = decode_json::<crate::access::http_api::ResponsesCreateRequest>(
            &request.payload,
    ...
    where
        ...
        ResponseHeader: Debug + Default + ZeroCopySend,
    {
        let req = decode_json::<crate::access::http_api::ResponseIdRequest>(
            &request.payload,
            "responses id",
        )?;
        match self.api.responses_get(req) {
            ...
            None => self.send_openai_error(
                ...
                &request,
                404,
                crate::access::http_api::OpenAiErrorResponse::bad_request("response not found"),
            ),
        }
    }
    ...
    where
        ...
        ResponseHeader: Debug + Default + ZeroCopySend,
    {
        let req = decode_json::<crate::access::http_api::ReloadModelsRequest>(
            &request.payload,
            "reload request",
        )?;
        match self
    ...
    fn send_openai_error<RequestHeader, ResponseHeader>(
        ...
        request: &IpcRequest,
        status_code: u16,
        error: crate::access::http_api::OpenAiErrorResponse,
    ) -> crate::Result<()>
    where
        RequestHeader: Debug + ZeroCopySend,
        ResponseHeader: Debug + Default + ZeroCopySend,
        ...
        ResponseHeader: Debug + Default + ZeroCopySend,
    {
        ...
        self.send_openai_error(
            ...
            request,
            status,
            crate::access::http_api::OpenAiErrorResponse::from_infer_error(&error),
        )
    }
